...
function removeVowels(inputStr) {
  ...
  for (var x in stringArr) {
    if (vowels.indexOf(stringArr[x]) === -1) {
      newStrArr.push(stringArr[x])
    }
  }
...
function startsWithVowel(inputStr) {
  var vowels = ["a", "e", "i", "o", "u", "A", "E", "I", "O", "U"]
  var stringArr = inputStr.split('')
  var vowelFirst = stringArr[0]
  var vowelStart = false

  if (vowels.indexOf(vowelFirst) !== -1) {
    vowelStart = true
  }
...
function endsWithVowel(inputStr) {
  var vowels = ["a", "e", "i", "o", "u", "A", "E", "I", "O", "U"]
  var stringArr = inputStr.split('')
  var stringLen = inputStr.length
  var vowelLast = stringArr[stringLen - 1]
  var vowelEnd = false

  if (vowels.indexOf(vowelLast) !== -1) {
    vowelEnd = true
  }
...
function startsAndEndsWithVowel(inputStr) {
  var vowels = ["a", "e", "i", "o", "u", "A", "E", "I", "O", "U"]
  var stringArr = inputStr.split('')
  var stringLen = inputStr.length
  var vowelFirst = stringArr[0]
...
function onlyPositiveNumbers (inputArr) {
  var newArr = [];
  for (var x in inputArr) {
    if (inputArr[x] > 0) {
      newArr.push(inputArr[x])
...
function onlyNegativeNumbers (inputArr) {
  var newArr = [];
  for (var x in inputArr) {
    if (inputArr[x] < 0) {
      newArr.push(inputArr[x]);
...
function hasEvens (inputArr) {
  var containedEven = false;
  for (var x in inputArr) {
    if (inputArr[x] % 2 === 0) {        //Check if even. If so, return true
      containedEven = true;
      return containedEven;
    }
  }
  return false;                          //If no even found, return false
}
...
function countNegatives (inputArr) {
  var negCount = 0;
  for (var x in inputArr) {
    if (inputArr[x] < 0) {
      negCount++;
...
function onlyPositiveEvens (inputArr) {
  var newArr = [];
  for (var x in inputArr) {
...
function onlyNegativeOdds (inputArr) {
  var newArr = [];
  for (var x in inputArr) {
    if ((inputArr[x] % 2 !== 0) && (inputArr[x] < 0)) {
      newArr.push(inputArr[x]);
...
function shortestString (inputArr) {
  var shortestStr = inputArr[0];
  for (var x = 1; x < inputArr.length; x++){
    if (inputArr[x].length < shortestStr.length) {
      shortestStr = inputArr[x];
    }
  }
  return shortestStr;
}
...
function getUniqueValues (inputArr) {
  ...
  for (var x = 1; x < inputArr.length; x++) {
    if (newArr.indexOf(inputArr[x]) === -1) {
      newArr.push(inputArr[x]);
    }
  }
  return newArr;
...
function getPaperTitle (inputObj) {
...
function getYearPublished (inputObj) {
  return inputObj.year_published;
}
...
function getAverageItemPrice (inputObj) {
  var totalPrice = 0;
  inputObj.items.forEach(function (itemObj) {
    totalPrice += itemObj.price
  })
  return totalPrice / numberOfItemTypes(inputObj);
}
...
function mostSpentOnItem (inputObj) {
  var firstObj = inputObj.items[0];
  var itemQty = firstObj.quantity;
  var itemPrice = firstObj.price;
  var highestSpent = itemQty * itemPrice;
  var returnObj = firstObj;
  for (var i = 1; i < inputObj.items.length; i++) {
    var checkObj = inputObj.items[i];
    if ((checkObj.quantity * checkObj.price) > highestSpent) {
      returnObj = checkObj;
    }
  }
  return returnObj;
